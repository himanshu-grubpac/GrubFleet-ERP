...
const kpis = [
  ...
  { label: "In Progress", value: "—", icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
  { label: "Completed Today", value: "—", icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50" },
  { label: "Overdue", value: "—", icon: AlertCircle, color: "text-red-600", bg: "bg-red-50" },
];

const columns = ["Job #", "Vehicle", "Type", "Assigned Tech", "Opened", "Est. Completion", "Status"];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Workshop</h1>
          <p className="text-sm text-slate-500">Track service jobs, technician assignments, and maintenance schedules.</p>
        </div>
        <button
          disabled
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div className="flex items-center gap-2">
            <ClipboardCheck className="h-4 w-4 text-[#FE5720]" />
            <h2 className="font-semibold text-slate-800">Service Job Queue</h2>
          </div>
          <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-500">0 jobs</span>
        </div>
            ...
            <tbody>
              <tr>
                <td colSpan={columns.length} className="px-6 py-16 text-center text-slate-400">
                  <Wrench className="mx-auto mb-2 h-8 w-8 text-slate-200" />
                  <p className="text-sm font-medium">No active jobs</p>
                  <p className="text-xs mt-1">Connect the Workshop API to populate the job queue.</p>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {priorities.map(({ label, cls }) => (
          <span key={label} className={`rounded-full px-3 py-1 text-xs font-medium ${cls}`}>{label}</span>
        ))}
      </div>
    </div>
  );
